...
export function LoadingState({ onComplete, answers, stage }: LoadingStateProps) {
    ...
    const analyzeAnswers = () => {
      ...
      const safetyStars = (() => {
        const hasBasicSafety = answers.safetyDevices?.includes('Smoke detectors') && 
  ...
  useEffect(() => {
    ...
    const progressInterval = setInterval(() => {
        ...
        const maxProgress = stage === 'generating' ? 50 : 
                         stage === 'saving' ? 90 : 
                         100;
              ...
              <span className="inline-flex items-center gap-0.5 text-amber-500">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    size="sm"
                    className={i < item.stars ? '' : 'opacity-30'}
                  />
                ))}
              </span>
            </div>
